...
workspace/runs/<project>/<timestamp>/
  ...
  output/
    ...
    protocol_metadata.json         # structured metadata (generated by post-processing script)
  scratchpad/
...
### Protocol metadata (post-processing)

After an ideation run completes, you can extract structured metadata into `output/protocol_metadata.json` using a standalone script:

```bash
python scripts/extract_metadata.py workspace/runs/default/20260408_175434_093820
```

The script reads the workspace files (protocol, critique report, principal notes, etc.), sends them to an LLM via LangChain structured output, and writes a JSON file with canonical, filterable fields — title, abstract, keywords, specialty, study type, topics, study design, references, ideation process scores, and more. The schema uses Pydantic models with `Literal` types to enforce controlled vocabularies for fields like `specialty` and `study_type`.

Options:

```bash
python scripts/extract_metadata.py <run_dir> --model claude-haiku-4   # use a faster/cheaper model
python scripts/extract_metadata.py <run_dir> --dry-run                 # print to stdout only
```
