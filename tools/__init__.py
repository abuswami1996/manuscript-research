"""Shared tools for Deep Agents: web search and Python execution."""
...
def get_workspace_dir() -> Path:
    """Active run directory (set by runtime before each invoke/stream)."""
    p = (
        os.environ.get("RUN_WORKSPACE", "").strip()
        or os.environ.get("MANUSCRIPT_RUN_WORKSPACE", "").strip()
    )
    if p:
